...
def savePhi(elapsed, fields_dir, counter):
    if parallelComm.procID == 0:
        fname = fields_dir / "phase_field_{}.tar.gz".format(counter)
        fname.touch()
    else:
        fname = None
...
def checkpoint(elapsed, counter):
    data_dir = Path('data')
    fields_dir = Path('data/fields')
    data_dir.mkdir(parents=True, exist_ok=True)
    fields_dir.mkdir(parents=True, exist_ok=True)
    saveStats(elapsed, data_dir)
    savePhi(elapsed, fields_dir, counter)
